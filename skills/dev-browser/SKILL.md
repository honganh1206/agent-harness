...
```bash
~/.agents/skills/dev-browser/server.sh &
```

Wait for the `Ready` message before running scripts. Add `--headless` flag if user requests headless mode.

**Key points:**
- Profile stored at `~/.agents/skills/dev-browser/profiles/browser-data`
...
```bash
cd ~/.agents/skills/dev-browser && npm i && npm run start-extension &
```
...
```bash
cd ~/.agents/skills/dev-browser && npx tsx <<'EOF'
